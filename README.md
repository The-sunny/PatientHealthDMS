...
on a purely local **MySQL** server. It ingests the [Synthea](https://synthetichealth.github.io/synthea/)
...
## Architecture (MySQL schemas standing in for cloud storage stages)

| Schema | Role |
|--------|------|
| *filesystem* | Raw Synthea CSVs sit untouched in a local folder before load. |
| `ehr_bronze` | Raw tables, 1:1 with source CSVs, all `VARCHAR`, no transformation. Audit columns `load_batch_id`, `loaded_at`. |
| `ehr_control` | **Sensitive.** Crosswalk (`original → surrogate` + date offset), reversible token vault, run log, DQ results, de-id audit. The only thing that can re-link the data. |
| `ehr_silver` | De-identified tables. Consistent per-patient ID swap + date shift applied across every patient-linked table. |
| `ehr_gold` | Cohort-ready analytics (`patient_summary` fact table). |
...
- **Tokenize** direct identifiers (name, SSN, license, passport, device UDI) via
  salted SHA-256, with a reversible vault in `ehr_control`.
...
- Every check logs pass/fail to `ehr_control.dq_results`.
...
## Prerequisites
- MySQL 9.x running locally (`brew services start mysql`).
